...
import { GATEWAY, MDM_ORDER, RUNTIME, orgFor } from "../data/install";
  ...
  },
  {
    ...
    render: ({ org }) => (
      <>
        ...
        <H3>Windows</H3>
        <P>Deploy <M>{RUNTIME.windows.msi.name}</M> as a Line-of-business app in Intune with a System install context. Kernel drivers are not required and not used: the Runtime runs as a Windows Service under LOCAL SYSTEM, attributes network flows with WFP filters, and confines launched agents inside an isolated account that has no route to the internet except through the Runtime's own egress proxy.</P>
    ...
    render: ({ org, bundleVersion }) => (
      <>
        ...
        <ol className="my-3 space-y-1.5 list-decimal pl-5 text-fg-2">
          <li>The Runtime generates a P-256 keypair in the platform keystore — the Secure Enclave on macOS, the TPM on Windows, and the OS keyring on Linux — and never exports the private key.</li>
    ...
    render: () => (
      <>
        ...
        <ul className="my-3 space-y-2 list-disc pl-5 text-fg-2">
          ...
          <li><B>Windows Defender flagged the shim.</B> The shim is signed with an EV certificate. Add an ASR exclusion for the Wrapbox install path, then run <M>wrapbox doctor</M> again.</li>
        </ul>
