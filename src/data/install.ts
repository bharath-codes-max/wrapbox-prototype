  ...
  windows: {
    ...
    intune: {
      ...
      steps: [
        ...
        "Set install context to System and install command to msiexec /i WrapboxRuntime.msi /quiet.",
        "Assign to the target device group. The Runtime enrols on first heartbeat.",
      ],
    },
  },
  linux: {
...
/** The Runtime's command for a given OS: install command, artifact, enrolment. Same values every surface reads. */
export function runtimeInstall(os: Os, org: string) {
  const enroll = RUNTIME.enroll(org);
  switch (os) {
    case "macos":
      return { artifact: RUNTIME.macos.pkg, install: `sudo installer -pkg ${RUNTIME.macos.pkg.name} -target /`, oneLiner: RUNTIME.macos.shell + ` && ${enroll}`, verify: RUNTIME.macos.verify(RUNTIME.macos.pkg), enroll };
    case "windows":
      return { artifact: RUNTIME.windows.msi, install: RUNTIME.windows.shell, oneLiner: null, verify: RUNTIME.windows.verify(RUNTIME.windows.msi), enroll };
    case "linux":
      return { artifact: RUNTIME.linux.tarball, install: RUNTIME.linux.apt, oneLiner: `${RUNTIME.linux.apt} && ${enroll}`, verify: RUNTIME.linux.verify(RUNTIME.linux.tarball), enroll };
  }
}

/** Guess the viewer's OS for the "Get started" default tab. */
export function guessOs(): Os {
  if (typeof navigator === "undefined") return "macos";
  const p = navigator.platform + " " + navigator.userAgent;
  if (/Win/.test(p)) return "windows";
  if (/Linux|X11/.test(p) && !/Android/.test(p)) return "linux";
  return "macos";
}
